...
package_name = "Default_Stuff"
parent_dir = os.path.dirname(current_dir)

if parent_dir not in sys.path:
    sys.path.insert(0, parent_dir)

Coins = importlib.import_module(f"{package_name}.Coins").Coins
Materails = importlib.import_module(f"{package_name}.Materials").Materails
Jobs = importlib.import_module(f"{package_name}.Jobs").Jobs
people = importlib.import_module(f"{package_name}.people").people
def GenResDay():
